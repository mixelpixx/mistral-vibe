...
class ReasoningMessage(SpinnerMixin, StreamingMessageBase):
    ...
    async def set_collapsed(self, collapsed: bool) -> None:
        ...
        if self._markdown:
            ...
            # Only render content when expanding (lazy rendering)
            ...
            elif collapsed and self._stream:
                await self._stream.stop()
                self._stream = None
                # Clear markdown content to free memory
                await self._markdown.update("")

    def stop_spinning(self) -> None:
        """Stop the spinner and cleanup timer."""
        super().stop_spinning()
